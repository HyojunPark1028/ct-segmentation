...
from einops import rearrange # einops는 torch.nn.functional.interpolate와 같은 방식으로 임포트되지 않습니다.
...
class PatchExpanding(nn.Module):
    def __init__(self, input_dim):
        ...
        self.output_dim = input_dim // 2 # input_dim이 2C이고, p1*p2=4이므로, 2C/4 = C/2 (input_dim/2)
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        # rearrange는 (B, H, W, 2*input_dim) -> (B, output_dim, H*2, W*2)
        # p1=2, p2=2 이므로 2*input_dim = p1*p2*c_out = 4 * c_out
        # 따라서 c_out = 2*input_dim / 4 = input_dim / 2
...
class SwinDecoderBlock(nn.Module):
    def __init__(self, in_dim, skip_dim, out_dim):
        super().__init__()
        self.up = PatchExpanding(in_dim) # PatchExpanding의 출력 채널은 in_dim // 2
...
class SwinUNet(nn.Module):
    def __init__(self, img_size=224, num_classes=1, use_pretrained=True):
        super().__init__()
        self.img_size = img_size # Add img_size to self for calculations
        ...
        # Decoder stages
        # SwinDecoderBlock(in_dim_from_prev_decoder_or_bottleneck, skip_dim_from_encoder, out_dim_of_this_decoder_stage)
        ...
        self.final = nn.Sequential(
            nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False), # Upsample to original image resolution
